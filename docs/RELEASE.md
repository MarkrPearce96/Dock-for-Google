...
4. When the run finishes, open **Releases** to download `Dock-for-Google-VERSION.dmg`. You do not need Xcode on your own Mac for this route.
...
- Only the DMG is uploaded directly to GitHub Releases.
...
and verifies a compressed DMG with an Applications shortcut, and writes only
`Dock-for-Google-VERSION.dmg` to `dist/vVERSION-dmg/`. Existing output directories
are never overwritten; move them aside before rebuilding the same version.
...
notes and attach only the DMG from `dist/vVERSION-dmg/`. Review and publish.
